...
- **7g Part A — complete (2026-07-09, commits `21816ae` A0 + `d4ac4dc` A1).**
  ...
  - **Ratified 7g design decisions (2026-07-09, planning thread):**
    1. **Two-score redefinition.** `variable_readiness_scores.technical_score` / `readiness_score` are redefined away from the Data Model §4.2 layer-bucket split (layer1–3 vs layer4–5 + device), under which five of six variables have an empty bucket feeding a NOT NULL column. Both scores become total functions of the variable's full check set. The Data Model §4.2 correction lands in the B1 doc commit.
    2. **Status-indicator weighted averaging.** Binary checks (`score: null`) enter variable aggregation as status indicators — PASS = 1.0, FAIL = 0.0 — weighted like any other check; NOT_APPLICABLE rows are excluded from the average, with weights renormalized over the contributing checks.
    3. **`check_results.threshold` is provenance metadata.** Recorded for audit/UI; the aggregator never re-applies it — status is check-owned. (Covers check 4's 0.97 metadata threshold and the derived-metric check's 0.02 tolerance, whose score is a deviation, lower-is-better, not a goodness fraction.)
    4. **VBC "PARTIALLY READY" ownership.** The Dataset-C VBC Reporting read reconciles at the site-level rollup owned by the use_case writer (46/49 patients ready), not in per-patient boolean stub aggregation (any-FAIL = NOT_READY stands per patient).
...
#### Dataset reload ledger (bug-set-extension window)
Consolidated dataset fixes to apply at the bug-set-extension reload, each tagged **BLOCKING** (must land for the acceptance gates to read correctly) or **FRAMING** (demo-narrative decision, not a correctness blocker).
- **BLOCKING (pre-7k) — 20 I10-active patients have no smoking observation in ANY session, including clean A** (PAT000002, 005, 007–013, 015, 018–021, 023, 027, 041–043, 045). Add LOINC 72166-2 observations for these patients in all three sessions so Dataset A goes clean and B/C fail exactly PAT000031–036. Until fixed, `layer1_notnull_fields_smoking` correctly reports A as not clean — **do not "fix" the check.**
