...
### Verified baseline — as of 2026-07-13, code HEAD `21f419b` (Step 8 Increment 5 complete), doc commits may sit on top (judged from code + DB, not from checkboxes)
...
- **7f — complete (7 of 7 checks; 2026-07-07/08/09, commits `cc5abad` + `da31307` + `b6e07dd` + `7a9401e` + `a1fce34` + `5503e7e` + `840deed`).** All 7f checks are fully config-driven (LOINC set from the variable's `code_references.loinc`, check-specific `params` on the check entry loaded via JSONB passthrough — no loader change, schema still V012 — diagnosis cohort from `condition.value_sets.diagnosis.codes`, anchor from `EVALUATION_DATE` as a YYYYMMDD string compare). Checks 1–2 are layer-1 presence + required-field audits (presence-ever semantics — recency belongs to other layers; `check_layer='layer1'` per the Data Model enum).
  ...
  - **`scoring/checks/layer3_mapped_values.js`** (check 5, `a1fce34`) — Bug 4, medications/RxNorm; the first care_coordination check. Cohort 49 (active DM-or-HTN from `population_definition`: pattern-or-exact code match with tail-only `.*` guard, strict `clinical_status = status_requirement`). Validity = membership in `params.valid_codes` (12-code config-owned set, shape-validated) — membership, not format: `999999`/`00000` are format-clean numerics only the set catches. Row scope is ALL medications regardless of `medications.status` (terminology validity is status-independent; seeded MED000378 is `'stopped'` and stays in scope by design). Score = valid/total, strict PASS at 1.0; zero med rows → NOT_APPLICABLE (policy implemented, unexercised — all 49 cohort patients have meds). `code_type` guard from `code_references.code_system` throws on non-rxnorm rows. Gate: A 49/0/0 all scores 1.0; B 46 PASS / 3 FAIL, FAILs exactly PAT000008 (12/13), PAT000015 (17/18), PAT000032 (7/8), each evidence naming its seeded invalid code; C identical to A (Bug 4 fully resolved); idempotent; `check_results` cleaned to 0.
    - `observed_value` JSON-evidence shape approved (deviation from the spec's bare-string wording for FAIL); the evidence-convention-convergence question is parked for 7g/fixture-export — CONSUMED at the fx window (see the Fixture-export block: convergence ratified at the export boundary).
...
- **7g Part B — complete (2026-07-09, commits `672e518` B0 + `0ba0c68` B1).**
  ...
  - **Standing fingerprints (whole-table content, both regression gates).** Standing rule (2026-07-11): **every standing fingerprint must record its query text verbatim, never prose-only.**
    - `check_results` — 1,500 rows @ `cf31fe218be90d9ec25a7d4c43cbcbaf`, canonical query recorded verbatim below. Supersedes `3da8be1a25bddde503f5b808f231f7dd` (1,245 rows; bug-set-extension re-baseline): query text unchanged; data + check set changed deliberately at the ext window — B reloaded on its fixed anchor (Add-1/Add-3 seeds + A1C recency backfill) + two new checks + ext-5b Encounter Record windowing — ext thread, 2026-07-11 (evidence gate_ext5_engine.txt / gate_ext5b_window.txt). Earlier: `3da8be1a…` superseded `3a9427a85cf8ed5be8a4c4cfae2c120b` (7k re-baseline: smoking coverage fix + PAT000041 A1C); `3a9427a8…` superseded `cdce162c94c7e7306e131805935c27b0` (7h gate disambiguation, 2026-07-11): the prior digest came from an A1-era query whose text was never recorded; ten plausible reconstructions did not reproduce it, with the data proven unchanged construction-independently (1,245 rows exact; all 33 check×session cells exact vs recorded gates; candidate digest byte-stable across the full 7h sequence including two complete engine runs, 2026-07-11). The defect was the unrecorded query text, not the data.
      ...
      ```
    - `variable_readiness_scores` — 876 rows @ `ffec28cbb65913ac9931248e58343010`, canonical query recorded verbatim below. Supersedes `5c329f5b58fb6f2a32c87c89c00cedab` (729 rows; bug-set-extension re-baseline: ext reload + ext-5 run + ext-5b windowing — new Encounter Record variable adds 49 rows/session; ext thread, 2026-07-11). Earlier: `5c329f5b…` superseded `e03468102a23dc19322f00ffa466a45f` (7k re-baseline: smoking coverage fix + PAT000041 A1C); `e0346810…` superseded `26894c67849f67bddea63a299b125069` (7h pre-check disambiguation, 2026-07-11): the prior digest came from a B1-era query whose text was never recorded; the pre-check reproduced the construction from prose and got a different digest with the data proven unchanged by four construction-independent surfaces (729 rows exact; all 34 variable×session×status cells exact vs recorded gates; named divergence-row signature exact; all 18 count cells exact). The defect was the unrecorded query text, not the data.
...
- **7h — complete (2026-07-11, commits `00e2284` fingerprint re-baseline docs + `69b4df4` code).** `scoring/lib/pathway_evaluator.js` + pathway stage wired into `scoring/index.js` (third stage, own transaction per session, after aggregation; per-use-case primary/fallback/no_valid_pathway summary; `loadUseCaseSpecs` now also selects `variable_pathways`). Implements the ratified 7h decisions (planning thread, 2026-07-11):
  ...
  - **Standing fingerprint added (third):** `use_case_pathway_results` — 507 rows @ `750539655bed8709c38bce1c6f87e3b7`, canonical query recorded verbatim below (all columns except pathway_result_id/evaluated_at, '∅' sentinel, '|' concat_ws, ';' agg, ORDER BY use_case_name, demo_session_id, patient_id). Supersedes `56b6a385fa522504bf0c45d0baddfd35` (bug-set-extension re-baseline: ext reload + ext-5 run + ext-5b windowing; ext thread, 2026-07-11). Earlier: `56b6a385…` superseded `1dc8b664db47d6979758ef3e02ff0a6a` (7k re-baseline: smoking coverage fix + PAT000041 A1C).
...
- **7i — complete (2026-07-11, commits `6a918d9` V014 + `3ede92a` code).** `scoring/lib/use_case_writer.js` + use-case readiness stage wired into `scoring/index.js` (fourth stage, own transaction per session, after pathway evaluation; per-use-case READY/PARTIALLY_READY/NOT_READY summary).
  ...
  - **Ratified 7i decisions (planning thread, 2026-07-11):**
    ...
    - **D3 — `required_variables`** = scoring pathway's variables (config order); blocking = subset NOT_READY (missing row counts as blocking, 7h D3 mirror); partial = subset PARTIALLY_READY; empty subsets written NULL, never empty arrays. Active-pathway semantics = Schema §7 Q5 working choice, parked for the clinical methodology lead (external academic partner) (`docs/methodology-open-questions.md` §10).
  ...
  - **Standing fingerprint added (fourth):** `use_case_readiness` — 507 rows @ `9109576d4b0e6e5fdfdfe6a74063509f`, canonical query recorded verbatim below (all columns except readiness_id/evaluated_at, '∅' sentinel, '|' concat_ws, ';' agg, ORDER BY use_case_name, demo_session_id, patient_id). Supersedes `a30ea7151dfce73a87c646560526e5e3` (bug-set-extension re-baseline: ext reload + ext-5 run + ext-5b windowing; ext thread, 2026-07-11). Earlier: `a30ea715…` superseded `f4c517dd2bbb38085e54f064ebd51bcb` (7k re-baseline: smoking coverage fix + PAT000041 A1C).
  ...
  - **CONSUMED (Step 8 inc4-c, 2026-07-13):** the broken `npm run build` self-cleared as predicted — the legacy wizard files are deleted and the build is green at `5c51890`. Do-not-repair held to the end.
  - **CONSUMED (Step 8 inc4-c, 2026-07-13):** the legacy package name is gone — `package.json` and both lockfile name fields now read `oros-ckm-data-readiness`.
  - **Public-release preparation COMPLETE (2026-07-18):** attribution migration, history review, and the visibility flip are done. See the Public release block below.
- **7j — complete (2026-07-11, commits `1dd3976` V015 + `63e88b5` code).** `scoring/lib/work_item_generator.js` + work-items stage wired into `scoring/index.js` (fifth stage, own transaction per session, after use-case readiness; per-use-case work-item summary).
  ...
  - **Standing fingerprint added (fifth):** `remediation_work_items` — 73 rows @ `dbf659d7c387edfa77a350d01934f5c5`, canonical query recorded verbatim below (content-pure: work_item_id, check_result_id, created_at excluded; '∅' sentinel, '|' concat_ws, ';' agg, ORDER BY check_name, demo_session_id, patient_id via the check_results join). Supersedes `f6e912d4f1d27ac87b56b5f3d6d3eb5f` (61 rows; bug-set-extension re-baseline: 12 new items from the Add-1/2/3 FAIL sets — B +9, C +3; ext thread, 2026-07-11). Earlier: `f6e912d4…` superseded `2e5a8b845ad18a38278865d0f9f0405e` (124 rows; 7k re-baseline — row count fell 124 → 61 because Dataset A's 20 dataset-gap smoking FAILs and PAT000041's A1C FAIL ×3 no longer existed).
...
- **Engine runs end-to-end through the work-items stage.** `node scoring/index.js --session all` executes five stages against all three sessions: thirteen checks, aggregation, pathway evaluation, use-case readiness, work items. `check_results` holds 1,500 rows, `variable_readiness_scores` 876, `use_case_pathway_results` 507, `use_case_readiness` 507, and `remediation_work_items` 73 — all five fingerprinted above. No runtime table remains empty.
- **Bug-set extension COMPLETE (ext window, 2026-07-11)** — Add-1/2/3 live; nine seeded bugs across 13 checks; Dataset A fully clean at check AND use-case levels. See the extension block below.
- **Fixture-export COMPLETE (fx window, 2026-07-12)** — see the Fixture-export block below.
...
- **Deliverables:** `scripts/fixture_content.mjs` (fx-1 — the single authored-content surface: recommendationType taxonomy, four-facts templates, per-check evidence renderers, surfaced criteria, site display band, workflow displayOrder, pipeline-stage copy; embedded `--selftest`) and `scripts/export_fixtures.mjs` (fx-2 — read-only exporter over the five runtime tables + config) → `src/data/fixtures/session-{a,b,c}.json`, the Step 8 `DATA_SOURCE='fixtures'` source.
- **Blocker grain:** one blocker per FAILing check per session (A 0 / B 11 / C 6), each carrying a content-map `bugId` hint (bug_1..bug_9 per the Reconciliation doc).
- **recommendationType taxonomy (ratified):** `ai_suggested_fix` ×3 (`layer3_mapped_values`, `layer2_value_standards`, `layer2_ranges_numeric_a1c`); session-dependent ×2 — both Bug 5 date checks flip `ai_suggested_fix` (B) → `route_to_stakeholder` (C), because the auto-normalizable records are fixed in C and only source-confirmation records remain; `route_to_stakeholder` ×6 (all others).
- **Four-facts split:** mechanical fields come from engine truth (counts, routing, displayName); authored `whatFailed`/`whatUnlocks` templates come from the content map with `{failCount}`/`{cohortSize}` interpolation, per-session C overrides (both date checks + temporal density), and a THROW on any FAILing check without a content entry — never a silent gap. `{evidenceExample}` is opt-in for exactly 7 checks (the five JSON-evidence checks + encounters structural + TIR); excluded elsewhere (counts only).
- **Site display band:** strict ready fraction (READY count/total); READY at 1.0, PARTIALLY_READY ≥ 0.85, else NOT_READY — uniform across use cases, content-map-owned (`BAND` in fixture_content), a demo display convention, NOT engine output. Sensitivity note (corrected expectation, post-7k): hypertension B/C = 29/35 = 0.8286 READY — just under the 0.85 line, so it reads NOT_READY; small dataset changes near the line flip the label.
...
- `src/data/provider.gate.test.ts` — 15 locked assertions vs the committed fixtures (blocker counts/order/routing/bugIds, site bands and patientCounts, diabetes-B quintet, pipeline stages, 46/27 FAIL rows, neon rejection). Gate: 15/15 green, scoped gate typecheck exit 0. Evidence: gate_inc1_precheck.txt, gate_inc1_build.txt, gate_inc1_build2.txt, gate_inc1_commit.txt (untracked).
...
The use-case front door per Demo UI/UX Spec §17 Increment 2. Gate G2-1..G2-17 all green: 31/31 tests, scoped gate typecheck exit 0, visual review passed on all three sessions. Evidence: gate_inc2_precheck.txt, gate_inc2a_harness.txt, gate_inc2b_tokens.txt, gate_inc2c_frontdoor.txt, gate_inc2_closeout.txt (untracked).
...
#### Step 8 Increment 3 — COMPLETE (2026-07-13, commits `0cd94d1` inc3-a + `54a1674` inc3-b + `a1408d4` inc3-c; protocol commit `4896680`)
Recommendation seam, remediation drawer, and legacy-agent deletion per Demo UI/UX Spec §17 Increment 3. Suite 48/48 and scoped gate typecheck exit 0 at close; fixtures untouched throughout (md5s A `d9731106…` / B `4a7d6d9a…` / C `b4416353…` unchanged at every gate). Evidence: gate_inc3_precheck, gate_inc3a_build, gate_inc3a_supp, gate_inc3a_commit, gate_inc3_evidence_move, gate_inc3b_build, gate_inc3b_commit, gate_inc3c_build, gate_inc3c_commit (all `.txt`, in `evidence/`).
- **3a (`0cd94d1`) — recommendation seam.** `src/recommendations/getRecommendation.ts` + `AGENT_MODE` (DemoConfig) per spec §10: scripted mode composes identity fields from the Blocker VERBATIM (passthrough, never re-derived); the live branch throws (designed, not built — no live.ts, Increment 1 neon precedent). `src/recommendations/scripted.ts` is the authored surface: 13 approved bodies (11 checks + the two Bug 5 C-side route_to_stakeholder variants), keyed by checkName with variant selection by the blocker's recommendationType; unknown checkName or entry/type mismatch THROWS named errors (fixture-drift guard — never a generic or silently-wrong recommendation). Gate R1–R9 (all 17 fixture blockers, Bug 5 flip, live/unknown/mismatch throws).
- **3b (`54a1674`) — RemediationDrawer + decisions + wiring.** Drawer on the harvested legacy shell STRUCTURE only (fixed right panel, no backdrop, header / scrollable body / footer), styled exclusively from theme tokens — the old palette and the dynamic `bg-${color}` class interpolation were not harvested. Real Approve/Reject (ai_suggested_fix only) into demo state; route_to_stakeholder renders role + action-required framing with no decision affordance; decided confirmations use the "routed as a versioned correction for review" register, never "fixed"; footer honesty label derives from AGENT_MODE via ImplementationBadge ("Demonstrated (stub)" in scripted). The FourFactsPanel affordance is the SOLE drawer trigger, coupled via demoState only (the trigger never imports the drawer); the drawer mounts once in App. Uniform checkName secondary text on every collapsed blocker row (list-driven); decided marker chip; readiness chips/card statuses untouched by decisions (G3b-4). Gate G3b-1..9 + visual review on B/C incl. the Bug 5 flip treatment pair.
- **3c (`a1408d4`) — legacy agent deletion + Archia scrub.** Deleted `src/agents/*` (4 files), `AgentInsightsDrawer.tsx`, `AskAnythingPanel.tsx` — 1,759 lines; scrubbed `enableMockArchia` + stale comments from `AppConfig.ts` and the legacy engine (4 lines). **Archia grep across src/ = ZERO, case-insensitive.** Gate G3c-1..7.
- **Ratified decisions (planning thread, 2026-07-13):**
  - **D1 — recommendation content home = `recommendations/scripted.ts`** (spec §5), NOT the fixture. Principle refined: **one authored surface per seam** — `scripts/fixture_content.mjs` owns readiness-render copy (travels in fixtures); `scripted.ts` owns recommendation copy (travels behind getRecommendation). If a funded phase gives recommendations an engine/DB home, the exporter picks them up; the seam is unchanged either way.
  - **D2 — decisions are session-keyed:** `${session}:${recommendationId}`, because fixture recommendationIds are CHECK-scoped (`rec_<checkName>`) and repeat across B and C; a B decision is invisible in C (G3b-5). Decisions are React state only, no persistence; reset (Increment 5) is the undo — **"reset clears decisions" is a TRANSFERRED gate assertion for Increment 5.**
  - **D3 — locked-assertion amendment principle:** an assertion that encodes a deliberately temporary state (G2-12's inert affordance) is amended in the increment that changes the state, with explicit planning-thread ratification.
  - **D4 — deletion boundary is grep-driven, not folder-driven.** Two knowingly-dangling imports remain in legacy files (`steps/AnalyticsStep.tsx` → AskAnythingPanel; `engine/DeterministicPipelineEngine.ts` → AgentHooks) — unreachable in dev, outside the scoped gate typecheck, **die at Increment 4**, at which point plain `tsc` resumes as the gate per the Increment 1 note.
- **Evidence protocol (commit `4896680`) — new standing convention:** all gate evidence is written to `evidence/` (git-ignored, never committed, never deleted; the permanent verification record). All earlier repo-root `gate_*.txt` / `precheck_*.txt` files were relocated into `evidence/` on 2026-07-13 — earlier §3 evidence references to "repo root" or bare "(untracked)" filenames now resolve there.
- **v0.1-copy ledger:** Increment 2 entry (b) — care_coordination duplicate-row disambiguation — **CONSUMED** (shipped as uniform checkName secondary text, 3b). **ADD:** the committed C-session whatFailed prose for the temporal-density blocker says "the platform's half is done" — brushes the local-team-attribution discipline; fold into the parked post-funding fixture-prose revision (same exporter-rerun cost class as the em-dash collision).
- **Increment 5 scope additions (first-audience UI review, 2026-07-13):** (a) session-identity signposting — an on-screen statement of what each session IS near the A/B/C selector; first-contact disorientation on B→C→B switching is the driving requirement; (b) demo-narration note: the C-session "deliberately not all green" beat (bugs 1/2/3/8 persist by design) needs explicit narration — rehearsal item; (c) the transferred "reset clears decisions" gate assertion (D2).
- **Copy verbosity:** deliberately NOT revised from n=1 self-report; revisit with real-audience feedback (recommendation bodies are cheap to revise — no fixture touch; fixture prose is the expensive class).

#### Step 8 Increment 4 — COMPLETE (2026-07-13, commits `26c7d01` inc4-a + `214c0f9` inc4-b + `e2ea4aa` inc4-b2 + `5c51890` inc4-c)
The under-the-hood pipeline view, the lead-view toggle, and the final legacy deletion per Demo UI/UX Spec §17 Increment 4. Suite 60/60 at close; `npm run build` green (first time since Increment 2); fixtures untouched all window (md5s A `d9731106…` / B `4a7d6d9a…` / C `b4416353…` at every gate). Evidence: gate_inc4_precheck, gate_inc4a_build, gate_inc4a_commit, gate_inc4b_preflight, gate_inc4b_build, gate_inc4b_commit, gate_inc4b2_build, gate_inc4b2_commit, gate_inc4c_build, gate_inc4c_commit (all `.txt`, in `evidence/`).
- **inc4-a (`26c7d01`) — PipelineView core.** `src/views/PipelineView/` (PipelineView + StageArc + ChecksPanel + stageAnnotations) + gate G4a-1..11. StageArc on the harvested legacy node-row PATTERN, theme tokens only (the legacy file was never imported; its statusColors map, gradient, dynamic class interpolation, and numbered-circle treatment were not harvested). Ratified: **D-A** — annotations live in a view-layer presentational map keyed by stageId; the fixture has no annotation field and must not gain one; unknown stageId renders unannotated, never an error. **D-B** — AI-assist marks at normalize (Architectural) + remediate (AGENT_MODE-derived: scripted → Demonstrated (stub), live → Implemented); Score carries "Deterministic core" / "No AI in scoring.", visually distinct from AI-assist. **D-C** — drill-down is data-presence-driven: a checks panel renders exactly where checkResults are non-empty (B 11 groups / 46 rows, C 6 / 27, A none), grouped by checkName, observedValue rendered VERBATIM. **D-D** — one-shot staggered CSS entrance; every stage arrives WITH its final status; prefers-reduced-motion renders static; the legacy running-state and animated connector-fill idioms explicitly not harvested. **Standing grep pattern:** the G4a-7-CORRECTION stage-literal pattern (object-key or quoted forms) is canonical going forward, scoped to rendering code — fixtures (stageIds as data), type-contract field names (the `score:` field in `CheckResultView`), and test files are out of scope (G4c-10 disambiguation).
- **inc4-b (`214c0f9`) — view toggle + LEAD_VIEW.** `LEAD_VIEW = 'use_case'` (the honest front door, spec §2.3); header toggle "Capabilities" / "Under the hood" in the session-selector idiom. Ratified: **R1** — pipeline selection closes the drawer, expandedBlockerId untouched, toggling never opens it; **R2** — view selection survives session switches (session-switch behavior otherwise unchanged); **R3** — `useDemoState(initialView = LEAD_VIEW)`, the gate-testability seam (Increment 1 precedent; App passes no argument). Prior 53 tests passed with ZERO edits; the anticipated G2-12 amendment concern DISSOLVED — that assertion was already amended at 3b and nothing asserted the toggle's absence. Gate G4b-1..9.
- **inc4-b2 (`e2ea4aa`) — visual-review fixes.** The arc renders all eight stages on one row (nowrap flex + horizontal scroll, tightened spacing); a status legend ("Complete" / "Needs attention" / "Pending") renders once beside the caption on the SHARED `stageFill` mapping (exported from StageArc; gate-asserted by DOM color equality against live stage nodes, never duplicated literals). Gate G4b2-1..7.
- **inc4-c (`5c51890`) — legacy deletion + gate transition.** The 19-file import-graph-locked set deleted (steps/ ×7, engine/ ×4, services/, types/wizard, wizard components ×3, wizardState, AppConfig, legacy logo png) — 3,063 lines, ZERO edits to any survivor; the two Increment-3 dangling imports died here; the root `tsc` 21-error baseline healed to exit 0. `tsconfig.gate.json` retired: **plain `tsc` (`npm run typecheck`) + `npm run build` are the gates.** **D-E ratified ride:** package renamed `oros-ckm-data-readiness` (package.json + both lockfile name fields). **G4c-7 deviation ratified:** the single residual old-name reference was this file's own parked rename note — consumed by this close-out. Gate G4c-1..10.
- **Increment 5 queue additions (extending the Increment 3 list):** (d) annotation-chip treatment redesign — first-encounter visual-review finding (2026-07-13): session-status dots and design-annotation chips read as one system; candidate treatments = merged muted chips vs a separate labeled strip below the arc; decision DEFERRED to Dominique's settled read after more UI time; (e) the "reset clears decisions" TRANSFERRED gate assertion (Increment 3 D2) is still pending — restated here so it survives the window.
- **Public-release note:** the release audit ran and the repository went public on 2026-07-18. See the Public release block below.

#### Step 8 Increment 5 — COMPLETE (2026-07-13, commits `29801e0` inc5-a + `192c5f2` inc5-b + `21f419b` inc5-c)
Reset, session-identity signposting, and annotation restructure per Demo UI/UX Spec §17 Increment 5. Suite 73/73 at close (60 at open); fixtures untouched all window (md5s A `d9731106…` / B `4a7d6d9a…` / C `b4416353…` at every gate); DB untouched. Evidence: gate_inc5_precheck, gate_inc5a_build, gate_inc5a_build2, gate_inc5a_build3, gate_inc5a_commit, gate_inc5b_build, gate_inc5b_supp, gate_inc5b_commit, gate_inc5c_build, gate_inc5c_commit (all `.txt`, in `evidence/`).
- **inc5-a (`29801e0`) — annotation restructure (hybrid, ratified).** The arc keeps EXACTLY ONE annotation chip (Score: "Deterministic core" / "No AI in scoring."); the two AI-assist annotations moved to a labeled strip below the arc (title "Where AI assists in this pipeline"; entries "Normalize: AI-assist" + badge, "Remediate: AI-assist" + badge, the Remediate badge AGENT_MODE-derived; framing sentence closes the loop: "The deterministic pipeline is the source of truth. AI assists at these two stages, a person approves every change, and scoring itself uses no AI."). Single authored source held: `stageAnnotations.ts` drives BOTH render slots (`arc_chip` / `ai_strip` kinds); no literal duplicated into a component (comment reworded in supplementary 2 to keep the grep single-file). Parens around badges removed (G5a-2 amended, D3-ratified): the ImplementationBadge pill is the delimiter. Gate G5a-1..6.
- **inc5-b (`192c5f2`) — session-identity signposting.** One line under the A/B/C selector states what the ACTIVE session is; updates on switch; renders in both views (selector-adjacent chrome). Strings are LOCKED approved copy in `src/views/shared/sessionSignpost.ts` (single source, exhaustive over SessionId): A "clean baseline", B "deliberately seeded data defects", C "after one remediation pass... routed to the people who can fix them". Driving requirement (B→C→B first-contact disorientation) verified in visual review. Gate G5b-1..5 (1–3 rendered assertions, 4–5 gate greps). The em-dash gate check is DIFF-SCOPED henceforth (the whole-file check false-positived on a pre-existing Increment 2 comment; check design corrected at inc5-c).
- **inc5-c (`21f419b`) — Reset demo + initial-session alignment.** **D5 ratified:** useDemoState initializes session to 'A' (was 'B'); a fresh load and a reset land on the identical known state. `reset()` on DemoState: session→A, decisions→{} cleared across ALL sessions, drawer closed, expandedBlockerId null, view→LEAD_VIEW (configured lead, not hard-coded). "Reset demo" is tertiary header chrome opposite the selector; no confirm dialog; NO new data path (re-point via getReadinessData only). The Increment 3 D2 TRANSFERRED assertion is CONSUMED: landed as G5c-2 (a B decision is gone after reset) and verified live in visual review. **Extended D3 amendment set ratified** (surfaced via STOP at pre-flight — the D5 blast radius exceeded the single authorized amendment): G5b-2 default-session assertion B→A; G5b-3's three default assertions B→A; drawer G3b-3 hook-default + state-grain keyDecision B→A (two lines, UI-grain untouched); pipeline `openAppB()` made session-explicit (helper clicks B; zero caller changes — G4b-2/G4b-4/G4b2-1/G5a-4 still exercise B). Gate G5c-1..5 (known state; D2 clearing; drawer close; arc-run-identical A→B→C→B twice; fresh-load equivalence).
- **v0.1-copy ledger:** pathwayNarrative candidate — DECIDED, DEFERRED to the funded-phase fixture-prose revision batch (with the em-dash collision and the "platform's half" attribution brush; same exporter-rerun + re-fingerprint cost class; population grain renders nothing pathway-related, so the copy has no committed render slot). Pending flag cleared. ADD: the shared headline ("Can this population's data support each capability?") renders above the pipeline view too — a mild §2.4 framing mismatch (fit-for-purpose question over the Foundational view); predates this window; copy-ledger candidate, no action taken.
- **Rehearsal / talk-track ledger** (oral, no on-screen support beyond the C signpost line, per ruling): (a) the C-beat — bugs 1/2/3/8 persist by design; "green everywhere would mean the system was hiding work; this screen is the work list"; (b) the B→C bridge — "my approval routes the correction; Session C is this population after that remediation pass has run" (the approve click and the C dataset are two ends of one story, connected by narration, not code); (c) the three-class B→C remediation breakdown — AI-suggested fixes human-approved (bugs 4, 9, the fixable 4-of-7 of bug 5); deterministic platform recomputation, no AI (bug 6: TIR/GMI from raw readings); stakeholder route closed upstream (bug 7: feed owner fixed structural fields) — while slow external loops stay open and routed (bugs 1, 2, 3, 8, bug 5 residual); (d) horizon beat at Unlock (deck framing, NOT demo chrome, NOT three-state labeled — roadmap only): human-initiated natural-language query over remediated data ("how many of my patients had an A1C over 9 in the past six months") — funded-phase / Step 9-adjacent candidate.
- **Forward items ratified this window:** **Step 10 deployment posture — Vercel with password protection is a MUST** (collaborator review infrastructure: the clinical methodology lead as core user/reviewer on their own time, feeding the parked scoring/remediation methodology questions; Lisa, Dan reviewers; narrated-first rule for pitch audiences — the link follows a framed conversation, never a cold first touch). Per-audience deck setup variants (funder / clinical / technical) are deck work, not build work. The public-release audit remains the leading next-window candidate; fold the 2026-07-14 Dan check-in outcomes into its sequencing.

#### Public release COMPLETE (2026-07-18)
The repository is PUBLIC (Apache 2.0) as of 2026-07-18. Release preparation, gate evidence in `evidence/` (git-ignored):
- **Internal working documents split out of the public tree.** The internal working set lives in `docs-private/` (git-ignored, internal working set, not published); the public `docs/` set is the nine build-reference documents. Registry rows and inline pointers in this file carry the "(internal working set, not published)" annotation.
...
#### Clinical Information Needs and Flows documentation family - COMPLETE (2026-08-03/04, commits `6660e1c` and `adb472d`)
Canonical clinical documentation family published to `docs/clinical-information-needs-and-flows/` (first subfolder in `docs/`). `6660e1c` (v1.0): core document and Companion C (Site Information Flow Specification Format). `adb472d`: core and Companion C revised to v1.1 (heading hierarchy nested under H1 titles, mission sentence reframed from a service claim to a dedication) and Companions A (Source and Transport Reference) and B (Discovery Template) added at v1.0. Markdown is canonical for this family; `.docx` versions are Drive working copies, not committed. All four files verified em-dash-free and free of individual names; sole organization mention is the Rising T1DE Alliance in dedication form. Companion cross-references to the Methodology Architecture and Operational Care Model point to documents deliberately not yet public, held pending naming consent (Gate 1) and collaboration agreements (Gate 2) per the publish sequencing ratified 2026-08-03.
...
#### Bug-set extension — COMPLETE (ext-1…ext-6, 2026-07-11; code commits `138932a` → `523c977`)
Add-1 (CSV-structural conformance), Add-2 (lab recency), Add-3 (value plausibility) are live: **nine seeded bugs across thirteen checks**, and Dataset A is fully clean at BOTH levels (zero FAIL check rows, zero non-READY use-case rows). Add-4 (DKA event validation) and denominator-validity remain funded-phase (see `docs/methodology-open-questions.md`). The extension carried its own dataset reload (B on its fixed §6 anchor — the second and final of the two ratified reloads) and the full five-fingerprint re-baseline above. Evidence: `gate_ext1_csv.txt` … `gate_ext6_closeout.txt` (untracked; relocated from repo root to `evidence/` 2026-07-13).
- **Sequence:** ext-1 CSV edits (Add-1 garbles `class`/`provider_id` → empty on ENC000392/395/401/408/421/427, B only; Add-3 value garbles OBS000095/185/255 → 81.0/93.0/66.0 `%`, B only; A1C recency backfill OBS001442–1448) → ext-2 surgical backfill append to A/C (`138932a`) → loader empty-string rule (`bda883e`) + `--session` re-attach reset path (`cee39e9`) → ext-3 B reset+reload on the fixed anchor (observations 1448/1439/1439, encounters 490) → ext-4 checks + configs (`364943b` `fitness_recency_a1c` + A1C weights rebalanced 0.40/0.24/0.16/0.20; `482e4bd` `layer1_notnull_fields_encounters` + vbc "Encounter Record" variable on `encounter_date_primary`) → ext-5 full engine run + locked gates → ext-5b Encounter Record windowing (`523c977`) → ext-6 close-out docs.
- **New-check signature ledger entries:**
  - `layer1_notnull_fields_encounters` (ehr/layer1, windowed to the vbc qualifying-encounters lookback, date-only predicate): A 49 PASS / 0 / 0 · B 46/3/0, FAILs exactly {PAT000042, 043, 045} · C 49/0/0. B FAIL evidence names exactly the in-window garbles (ENC000395/408/427); the pre-window garbles (ENC000392/401/421) exist in the data but sit outside the audit by design. PAT000031 (encounters all pre-window) is outside the cohort.
  - `fitness_recency_a1c` (ehr, layer NULL — fit-for-purpose recency; presence stays layer1's, zero-rows → N_A): A 36/0/0 · B 33/3/0 · C 33/3/0, FAILs {PAT000012, 022, 050} in both B and C (unresolved in C by design — a stale lab needs collection, not data repair). Inclusive window boundary is load-bearing: PAT000001/003 sit exactly on 20240514 and PASS.
  - **Updated:** `layer2_ranges_numeric_a1c` — B now 33/3/0, FAILs {PAT000011, 023, 049} (Add-3 seeds); A and C 36/0/0 (resolved in C).
  - **Note:** `layer1_notnull_fields_a1c` and `layer2_ranges_numeric_a1c` A/C rows re-baselined at ext-5 with **statuses unchanged** — evidence strings only (the ext-2 backfill blast radius: A 7 patients, C 4; the pre-ext runtime rows predated the backfill).
- **ext-5b windowing decision:** the Encounter Record cohort and audited rows are scoped to the vbc reporting window (lookback read from `population_definition.qualifying_encounters`, never hardcoded; DATE-ONLY predicate — audited fields never filter the audit). This removed the eligibility-vs-readiness conflation that had pulled PAT000031 into the vbc row set as NOT_READY across all sessions including clean A (7h D4 union geometry). Union-geometry vs population_definition reconciliation for stub use cases parked to `docs/methodology-open-questions.md` §11.
- **Work items 73** (A 0 / B 46 / C 27); new tuples exactly (`layer1_notnull_fields_encounters`, B) × {042,043,045} → Structural Feed Non-Conformance / Regional Data Node; (`fitness_recency_a1c`, B and C) × {012,022,050} → Stale Required Lab / Primary Care Site; (`layer2_ranges_numeric_a1c`, B) × {011,023,049}.
- **Learnings (ext window):**
  1. Occupied-target sets for seeding must compile from the Bug Reconciliation doc, not from §3 check signatures — the signatures list FAILing patients, not occupied rows.
  2. Cohort shrink under upsert-only writers strands rows (no writer has a deletion path) — any spec that shrinks a cohort must pre-authorize the runtime wipe (7k precedent, applied again at ext-5b).
  3. B reload wall-clock ≈ 35 min, dominated by `cgm_readings` row-by-row inserts — COPY-based loading is a funded-phase optimization.
  4. The loader's internal expected-count table is stale (1414-era observations counts) — fix parked to the environment-touch batch; exercising it costs a reload.
  5. Commits now carry `Co-Authored-By` per current session convention (ext-2/ext-3 predate it).

#### Open methodology questions (separate from the build)
Open methodology questions (weight basis, device-linkage classification, terminology, Add-4) are tracked in `docs/methodology-open-questions.md` — revisited in a dedicated methodology-triage pass with the clinical methodology lead, separate from the build.
...
└── checks/
    ...
    ├── fitness_recency_a1c.js                 (ext Add-2 — built; A1C recency, scope ehr, layer NULL)
    └── layer1_notnull_fields_encounters.js    (ext Add-1 — built; encounters structural conformance, windowed)
...
- **A1C check weights (rebalanced at ext Add-2):** `layer1_notnull_fields_a1c` 0.40, `layer2_ranges_numeric_a1c` 0.24, `layer5_date_concordance_a1c` 0.16, `fitness_recency_a1c` 0.20
...
npm run typecheck  # plain tsc — the typecheck gate since inc4-c (tsconfig.gate.json retired)
npm run build      # tsc && vite build — green since inc4-c; must stay green
npm run test:run   # vitest suite (60 tests at Increment 4 close)
```
