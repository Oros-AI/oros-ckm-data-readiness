...
### Verified baseline — as of 2026-07-08, code HEAD `7a9401e`, doc commits may sit on top (judged from code + DB, not from checkboxes)
...
- **7f — in progress (4 of 7 checks; 2026-07-07/08, commits `cc5abad` + `da31307` + `b6e07dd` + `7a9401e`).** All 7f checks are fully config-driven (LOINC set from the variable's `code_references.loinc`, check-specific `params` on the check entry loaded via JSONB passthrough — no loader change, schema still V012 — diagnosis cohort from `condition.value_sets.diagnosis.codes`, anchor from `EVALUATION_DATE` as a YYYYMMDD string compare). Checks 1–2 are layer-1 presence + required-field audits (presence-ever semantics — recency belongs to other layers; `check_layer='layer1'` per the Data Model enum).
  ...
  - **Carry-forward — Dataset B has one fewer E11.9 condition row than A/C (89 vs 90).** No cohort effect (dm cohort is 36 in all sessions); almost certainly a Bug 4 invalid-terminology replacement of a DM condition row. Identify the row in the next thread's (Bug 4 pair) pre-check.
- **Remaining checks not built.** Three 7f checks remain — next the Bug 4 pair (`layer3_mapped_values` + `layer2_value_standards`), then `layer5_date_concordance` (Bug 5). The config loader warns only for those.
- **Engine not end-to-end runnable.** `check_results` HAS been written and verified by the layer6, 7e, 7e2, and 7f-checks-1–4 behavioral gates, then cleaned back to 0 rows for a clean baseline (all eight checks are committed and re-runnable). `variable_readiness_scores`, `use_case_readiness`, and `remediation_work_items` remain empty. The scoring orchestrator (`scoring/index.js`), aggregator, pathway evaluator, use_case writer, and work-item generator do not exist yet.
